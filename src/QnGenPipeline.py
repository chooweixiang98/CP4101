    ...
    **kwargs,
    ...
    if isinstance(tokenizer, (str, tuple)):
        if isinstance(tokenizer, tuple):
            ...
            tokenizer = AutoTokenizer.from_pretrained(tokenizer[0], use_fast=False, **tokenizer[1])
        else:
        ...
        if ans_model is None:
            ...
            if isinstance(ans_tokenizer, (str, tuple)):
                if isinstance(ans_tokenizer, tuple):
                    # For tuple we have (tokenizer name, {kwargs})
                    ans_tokenizer = AutoTokenizer.from_pretrained(ans_tokenizer[0], use_fast=False, **ans_tokenizer[1])
                else:
                    ans_tokenizer = AutoTokenizer.from_pretrained(ans_tokenizer, use_fast=False)

            if isinstance(ans_model, str):
                ans_model = AutoModelForSeq2SeqLM.from_pretrained(ans_model, use_fast=False)
